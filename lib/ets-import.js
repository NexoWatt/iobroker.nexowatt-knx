 ...
 * @param {string} etsFileName file name inside ioBroker Files -> nexowatt-knx
...
async function importEtsProject(adapter, etsFileName, options = {}) {
  ...
  // Read from ioBroker file storage
  const fileName = etsFileName.trim();
  const fileObj = await adapter.readFileAsync(adapter.name, fileName);
  const file = fileObj?.file;
  if (!file) {
    throw new Error(`Could not read ETS project from ioBroker Files: ${adapter.name}/${fileName}`);
  }
